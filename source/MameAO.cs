...
";
		...
		public void BringToFront()
		{
			if (ConsoleHandle == IntPtr.Zero)
				AnsiConsole.MarkupLine("!!! Wanring can't get handle on Console Window.");
			else
		...
		public void Initialize()
		{
			...
			AnsiConsole.Markup(WelcomeText.Replace("@VERSION", Globals.AssemblyVersion));
			...
			if (config.ContainsKey("MameVersion") == true)
			{
				...
				AnsiConsole.MarkupLine($"MAME Version fixed: {Globals.MameVersion}");
			}
			else
			{
			...
			if (File.Exists(binFilename) == false)
			{
				...
				{
					AnsiConsole.Markup($"Downloading MAME binaries {binUrl} ...");
					Tools.Download(binUrl, binCacheFilename);
					AnsiConsole.MarkupLine("...done.");
				}

				AnsiConsole.Markup($"Extracting MAME binaries {binFilename} ...");
			...
			{
				AnsiConsole.Markup($"Extracting MAME machine XML {machineXmlFilename} ...");
			...
			if (File.Exists(softwareXmlFilename) == false)
			{
                ...
                AnsiConsole.Markup($"Extracting MAME software XML {softwareXmlFilename} ...");
		...
		public void Shell()
		{
            ...
            {
                FileName = Globals.ListenAddress,
                UseShellExecute = true, // Use the shell to execute the process
                WorkingDirectory = Globals.RootDirectory // Set the working directory
                //WorkingDirectory = @"C:\Users\mortl\source\repos\mame-ao-assist\mame-ao-assist\bin\Debug\net9.0-windows10.0.26100.0" // Set the working directory
            };
            Process.Start(psi);
			...
			while (true)
			{
				AnsiConsole.Markup($"MAME Shell ({Globals.MameVersion})> ");
